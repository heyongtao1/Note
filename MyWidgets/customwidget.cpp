    ...
    CustomDialog(parent)
{
    ...
    labelWinIcon->setPixmap(QPixmap(":/resources/image/ico/镜像设置.png"));
    ...
    labelWinTitle->setStyleSheet(QString("QLabel {color:#ffffff;}"));

    btnWinMin   = new QPushButton(this);
    btnWinMin->setObjectName("btnWinMin");
    btnWinMin->setIcon(QIcon(":/resources/image/comimage/ic_min_white.png"));

    btnWinClose = new QPushButton(this);
    btnWinClose->setObjectName("btnWinClose");
    btnWinClose->setIcon(QIcon(":/resources/image/comimage/ic_close_white.png"));
    ...
    CBaseDialog(parent)
{
    ...
    this->setStyleSheet("background-color:#3b3b3b;border-radius:5px;");
    //this->setAttribute(Qt::WA_TranslucentBackground);
    //->setStyleSheet("border-radius:7px;");
    labelIcon = new QLabel(widgetBody);
    labelIcon->setFixedSize(QSize(64, 64));
    labelIcon->setAlignment(Qt::AlignCenter);
    labelIcon->setPixmap(QPixmap(":/Image/Login/信息提示.png"));
    ...
    labelMsgContent->setStyleSheet(QString("QLabel {font: 12px;color:#ffffff;}"));
    ...
    btnOk->setStyleSheet(QString("QPushButton {color:#ffffff;}"));
    btnOk->setText(tr("确定"));
...
void CMessageBox::ShowMessage(const QString &content, const quint8 &msgType, const QString &title)
{
    ...
    if (CMessageBox::Information == msgType)
         labelIcon->setPixmap(QPixmap(":/Image/Login/xxtis.png"));
    else if (CMessageBox::Warnings == msgType)
        labelIcon->setPixmap(QPixmap(":/Image/Login/警告.png"));
    else if (CMessageBox::Questions == msgType)
        labelIcon->setPixmap(QPixmap(":/Image/Login/同意合并请求.png"));
    else
        labelIcon->setPixmap(QPixmap(":/Image/Login/其他.png"));
    labelIcon->update();
}
...
{
   btnOk->setText(QStringLiteral("确定(%1)").arg(m_nTimerCnt--));
   if (m_nTimerCnt < 0) {
       m_timer->stop();
       this->accept();
   }
